...
const Dashboard = () => {
  ...
  const [searchQuery, setSearchQuery] = useState('');
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [showVerifiedOnly, setShowVerifiedOnly] = useState(false); // State for filtering verified shops
  const location = useLocation();
  const userId = useSelector((state) => state.login.myUserid);
  ...
  // Filter shops based on search query and verification status
  const filteredShops = shops.filter((shop) => {
